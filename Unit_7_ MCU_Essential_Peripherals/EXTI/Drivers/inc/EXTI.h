...
typedef struct
{
    ...
    uint8_t                  Trigger_Case ; //Specifies Rising or falling or both trigger
     ...
     uint8_t                  IRQ_EN    ; //Enable OR Disable the EXTI IRQ (that will enable the IRQ MASK IN EXTI and also on tje NVIC interrupt controller)
                                          //This Parameter must be set based on @ref EXTI_IRQ_define
     void(* P_IRQ_Callback)(void) ;       //Set the c function() which will be called once IRQ happen
